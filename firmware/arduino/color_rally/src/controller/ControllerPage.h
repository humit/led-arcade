...
async function enableMotion(){try{const details=[];details.push("secure="+window.isSecureContext);details.push("origin="+location.origin);const permissions=[];if(typeof DeviceMotionEvent!=="undefined"&&typeof DeviceMotionEvent.requestPermission==="function"){permissions.push("motion="+await DeviceMotionEvent.requestPermission())}if(typeof DeviceOrientationEvent!=="undefined"&&typeof DeviceOrientationEvent.requestPermission==="function"){permissions.push("orientation="+await DeviceOrientationEvent.requestPermission())}if(permissions.some(v=>!v.endsWith("=granted")))throw new Error(permissions.join(", ")+" · "+details.join(" · "));window.addEventListener("deviceorientation",onOrientation,true);motionEnabled=true;motionBtn.textContent="HAREKET AKTİF";statusEl.textContent="Telefonu sağa-sola eğerek topu hareket ettir. · "+details.join(" · ")}catch(e){motionEnabled=false;statusEl.textContent="Hareket izni alınamadı: "+e.message}}
